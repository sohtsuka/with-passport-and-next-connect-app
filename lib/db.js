...
export function createVerification(req, { username, email, code, expiresAt }) {
  const verification = {
    id: uuidv4(),
    createdAt: Date.now(),
    expiresAt: expiresAt,
  ...
  // Here you should insert the user into the database
  // await db.createUser(user)
  req.session.verifications.push(verification)
}

export function findVerificationByCode(req, code) {
  return req.session.verifications.find((verification) => verification.code === code)
}

export function findOriginUserByUsername(req, username) {
  return req.session.originUsers.find((originUser) => originUser.username === username)
}
